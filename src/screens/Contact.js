        ...
        <TransitionGroup component={React.Fragment}>
          ...
          {complete && (
            <Transition appear timeout={0} mountOnEnter unmountOnExit>
              {(status) => (
                <ContactComplete>
                  <ContactCompleteTitle status={status} delay={0}>
                    Спасибо {name}! Ваш заказ отправлен
                  </ContactCompleteTitle>
                  <ContactCompleteText status={status} delay={200}>
                    Ваш будущий сайт слишком хорош, чтобы принадлежать кому-то
                    другому
                  </ContactCompleteText>
            ...
            <Transition appear timeout={0} mountOnEnter unmountOnExit>
              {(status) => (
                <ContactComplete>
                  ...
                  <ContactCompleteText status={status} delay={200}>
                    {name}! Свяжитесь снами по телефону <b>0632948902</b>, или
                    попробуйте позже
                  </ContactCompleteText>
                  <ContactCompleteButton
                    secondary
...
const ContactButton = styled(ProjectHeaderButton)`
  ...
  transition-duration: ${(props) =>
    props.status === "entered" ? "0.4s" : "0.8s"};
  transform: translate3d(0, 80px, 0);
  opacity: 0;
